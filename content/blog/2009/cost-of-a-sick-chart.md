...
Among its many divisions and units is [GE Healthcare](http://en.wikipedia.org/wiki/GE_Healthcare), which employs 46,000 people. Just like GE in general, the latter develops lots of things, like imaging products, patient data systems, tools for drug discovery and manufacturing, surgery tools, and many more. When such a company decides to visualize data for the masses, we should be able to expect something substantial.

The actual work was done by (or at least under the supervision of) [Ben Fry](http://benfry.com/). He is the creator (together with Casey Reas) of the [Processing](http://processing.org/) programming language and environment that is very popular with media artists, and the author of a book on information visualization using the language. His [zipdecode](http://benfry.com/zipdecode/) project was the inspiration for my [ZIPScribble Maps](/app/zipscribble-map). Fry is also the director of [Seed Visualization](http://seedmediagroup.com/visualization/), which is the company GE Healthcare contracted this visualization out to. GE has put up [a video of Fry talking about data visualization](http://www.youtube.com/gereports#p/u/0/zN-ZehcVDsI), though it only mentions this piece in passing.
...
Costs are encoded in the radius of each pie slice (the mapping takes [the quadratic growth of area](/blog/2008/linear-vs-quadratic-change) into account, but see below). The angle of the slice is the number of cases in GE's dataset that covers around 500,000 people. Divided into individual ages and diseases, that still leads to fairly small numbers per pie slice, though.

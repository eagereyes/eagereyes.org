# Link: The Graphic Continuum

*[The Graphic Continuum](http://policyviz.com/graphic-continuum/)* is a poster created by [Jon Schwabish](http://policyviz.com/) and Severino Ribecca (the man behind the *[Data Visualisation Catalogue](http://www.datavizcatalogue.com)*). It lists almost 90 different chart types and organizes them into five large groups: distribution, time, comparing categories, geospatial, part-to-whole, and relationships. Some of them are connected across groups where there are further similarities.

The poster is printed very nicely and makes for a great piece of wall art to stare at when thinking about data, and maybe to get an idea for what new visualization to try.

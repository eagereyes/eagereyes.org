...
Being part of the DOM also has its disadvantages, though: updates require more work and take longer. This is why visualizations using [protovis](http://protovis.org/) can feel sluggish when they need to update a lot (protovis uses SVG for the actual rendering).
...
While JavaScript used to be slow for the first decade or so of its existence, the last year or two have brought some amazing advances in speed. Today, JavaScript is compiled into native code in Chrome, Safari, and FireFox, and is easily on par with vm-based languages like Java. There are now even ways to [run JavaScript on the server](http://nodejs.org/), and it is conceivable that it will become a standard scripting language for many applications soon.
...
While there is a lot of information about SVG out there, things are a bit more difficult to find for canvas. Mozilla has [a nice, basic tutorial](https://developer.mozilla.org/en/canvas_tutorial), and Mark Pilgrim [covers some additional ground in his online book](http://diveintohtml5.org/canvas.html). Hakim El Hattab has [some really impressive game demos](http://hakim.se/experiments/).

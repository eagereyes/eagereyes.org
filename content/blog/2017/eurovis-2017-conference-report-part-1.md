...
One interesting little tidbit this time was the comparison to when EuroVis (then still called *VisSym*) was in Barcelona before in 2002: then it was just over 60 attendees, this year’s is over 300.

The keynote made up for the boring parts, though. [Fernanda Viégas](https://research.google.com/pubs/FernandaViegas.html) and [Martin Wattenberg](https://research.google.com/pubs/MartinWattenberg.html), who have been with Google Research for a number of years now, talked about *Visualization: The secret weapon of machine learning*.

They started with a brief introduction to some basic ideas in machine learning, including that neural networks are not actually meant to model the structure of the human brain. They only mirror some aspects of its structure that seem to work well for many tasks. Martin’s [simple chess program that you can watch think](http://www.bewitched.com/chess/) made an appearance, and while I’ve seen it several times now, it’s always fun to watch.

Martin and Fernanda (I’m still trying to establish the collective term *Marnanda* for them, because they virtually always give talks together) then talked about some of their more exciting work, like the [TensorFlow Playground](http://playground.tensorflow.org). This project, which is now a few years old, lets people play with neural networks to build classifiers. It looks simple, but it allows you to change many parameters that are quite cumbersome to change in the more traditional way. This lets people experiment much more quickly than before and see what is going on, which makes this great for teaching (and it’s used now within Google as a teaching tool for new machine learning folks).

A more recent piece is the [Embedding Projector](http://projector.tensorflow.org), which lets you explore projection approaches for high-dimensional spaces, like PCA or t-SNE. It’s a clever, interactive piece that I haven’t explored much yet, but that seems quite powerful.

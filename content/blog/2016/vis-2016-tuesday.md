<figure><img src="https://media.eagereyes.org/wp-content/uploads/2016/10/vis16-tuesday.jpg" width="720" height="540"/></figure>
...
[Hausmann is an economist at Harvard](https://www.hks.harvard.edu/about/faculty-staff-directory/ricardo-hausmann) and runs the group behind the [*The Atlas of Economic Complexity*](http://atlas.cid.harvard.edu). The Atlas is the embodiment of Hausmann's theory that the wealth gap between nations (and also between regions within countries) can be modeled by network effects.
...
[*Vega-Lite: A Grammar of Interactive Graphics*](https://idl.cs.washington.edu/papers/vega-lite) by Arvind Satyanarayan, Dominik Moritz, Kanit Wongsuphasawat, and Jeffrey Heer received the Best Paper award this year. Vega Lite is a sort of successor to Vega, which allows you to specify visualizations in a declarative way. Vega Lite is much more concise however, because it fills in defaults for things that are not specified, and provides many complex things out of the box (like a selection based on a Voronoi diagram, zooming and panning, brushing, etc.)

Vega is open source, and there is [an online playground](http://vega.github.io/vega-lite/) to try it out. This is based on an earlier version than the paper presented though, but they hope to release an updated version soon. They are also seeing others build infrastructure on top of Vega-Lite, like [a set of friendly Python bindings called *Altair*](https://github.com/ellisonbg/altair).

[*HindSight: Encouraging Exploration through Direct Encoding of Personal Interaction History*](http://wpivis.github.io/hindsight) by Mi Feng, Cheng Deng, Evan M. Peck, and Lane Harrison shows users their own histories to increase interaction and engagement. This is somewhat similar to the way browsers indicate visited links, but richer and with more of an idea of an Information scent.
